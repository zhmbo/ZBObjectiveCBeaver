...
#import <ZBObjectiveCBeaver/ZBServerDestination.h>
#import <ZBObjectiveCBeaver/ZBAES256.h>
...
- (instancetype)initWithAppID:(NSString *)appID
    ...
    return [self initWithAppID:appID
                 ...
                 encryptionKey:encryptionKey
                     serverURL:[NSURL URLWithString:@"https://api.swiftybeaver.com/api/entries/"]
    ...
    if (!_sendingInProgress) {
        ...
        if (lines > 0) {
            ...
            if (str && ![str isEqualToString:@""]) {
                ...
                [self toNSLog:[NSString stringWithFormat:@"Encrypting %ld log entries ...", (long)lines]];
                
                NSString *encryptedStr = [self encrypt:str];
                if (encryptedStr.length > 0) {
                    [self toNSLog:[NSString stringWithFormat:@"Sending %ld encrypted log entries %ld chars) to server ...", (long)lines, encryptedStr.length]];
                    __weak __typeof__(self) weakSelf = self;
                    [self sendToServerAsync:encryptedStr complete:^(BOOL ok, NSInteger status) {
                        [weakSelf toNSLog:[NSString stringWithFormat:@"Sent %ld encrypted log entries to server, received ok: %d", lines, ok]];
                        if (ok) {
...
- (void)sendToServerAsync:(NSString *)str complete:(void(^)(BOOL ok, NSInteger status))complete {
    ...
    if (str.length > 0 && self.queue != nil && _serverURL != nil) {
        ...
        [NSMutableURLRequest requestWithURL:_serverURL cachePolicy:NSURLRequestReloadIgnoringLocalAndRemoteCacheData timeoutInterval:timeout];
        ...
        [request setValue:[@"Basic " stringByAppendingString:base64Credentials] forHTTPHeaderField:@"Authorization"];
            ...
            if (error) {
                ...
                if (response) {
                    ...
                    if (status == 200) {
                        ...
                        [self toNSLog:[NSString stringWithFormat:@"Error! Sending entries to server failed with status code %ld", status]];
                    }
                }
            }
            if (complete) {
...
- (NSString *)encrypt:(NSString *)str {
    return [str aes256_encrypt:_encryptionKey];
}
